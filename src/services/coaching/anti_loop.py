"""Изолированный stabilizing policy-слой для детекции зацикливания диалога.

Назначение модуля:
- анализировать только динамику диалога;
- детектировать loop/stagnation детерминированно;
- вернуть структурированное решение по следующему типу сдвига.

Ограничения:
- без интеграции в handler-flow и UI;
- без вмешательства в safety/premium/growth;
- без ML/NLP/LLM и без смысловой интерпретации пользователя.
"""
    ...
    clear_condition = (
        ...
        or (
            ...
            and (
                ...
                or flags.short_repetitive_answers
            )
        )
...
def _topic_switch_pending(state: SessionState) -> bool:
    ...
    return bool(
        topic_id
        and pending_topic_id
        and pending_topic_id != topic_id
    )
