...
def _compose_domain_response(
    ...
    if response_plan.reply_type == "coach_message":
        step_templates: dict[str, tuple[str, str]] = {
            "structured_progress": (
                "Похоже, в этой ситуации уже можно выделить одну рабочую опору.",
                "Что в этой теме сейчас наиболее точно описывает ваш главный узел?",
            ),
            "state_clarification": (
                "Похоже, в этом месте могут смешиваться несколько смыслов сразу.",
                "Какой фрагмент вопроса лучше прояснить первым?",
            ),
            "clarity_structuring": (
                "Похоже, здесь полезно аккуратно разложить ситуацию на части.",
                "С чего начнем уточнение: с фактов ситуации "
                "или с вашей текущей реакции?",
            ),
            "emotion_contact": (
